...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.BY_TIME, query = "SELECT m FROM Meal m where m.user.id=:user_id " +
                "and (:startTime is null or m.dateTime>= :startTime)" +
                "and (:endTime is null or m.dateTime<:endTime) ORDER BY m.dateTime DESC"),
})
@Entity
